...
from cgd.util import resize_image
import clip
...
@lru_cache(maxsize=1)
def load_clip(model_name='ViT-B/32', device="cuda"):
    if device == "cpu":
        clip_model = clip.load(model_name, jit=False)[
            0].eval().to(device=device).float()
    elif "cuda" in device:
        clip_model = clip.load(model_name, jit=False)[
            0].eval().requires_grad_(False).to(device=device)
    else:
...
def clip_encode_text(model_name, text, device='cuda', truncate: bool = True):
    clip_model, _ = load_clip(model_name=model_name, device=device)
...
def imagenet_top_n(prompt: str = '', prompt_min: str = '', min_weight: float = 0.1, device: str = 'cuda', n: int = len(IMAGENET_CLASSES), clip_model_name: str = "ViT-B/32"):
    """
    ...
    with th.no_grad():
        ...
        return text_probs.cpu().topk(n, dim=-1, sorted=True).indices[0]